...
public:
    ...
    // Add to the public section of the motor_group class in motors.h:

/**
 * @brief Turn on the motors and spin them to an absolute target position value at a specified voltage.
 * @return Returns a Boolean that signifies when the motor has reached the target position value.
 * @param position Sets the target absolute position.
 * @param units The measurement unit for the position value.
 * @param voltage Sets the amount of voltage.
 * @param units_v The measurement unit for the voltage value.
 * @param waitForCompletion (Optional) If true, your program will wait until the motor reaches the target position. If false, the program will continue after calling this function. By default, this parameter is true.
 */
bool spinToPosition(float position, vex::rotationUnits units, float voltage, vex::voltageUnits units_v, bool waitForCompletion=true);

/**
 * @brief Turn on the motors and spin them to an absolute target position value.
 * @return Returns a Boolean that signifies when the motor has reached the target position value.
 * @param position Sets the target absolute position.
 * @param units The measurement unit for the position value.
 * @param waitForCompletion (Optional) If true, your program will wait until the motor reaches the target position. If false, the program will continue after calling this function. By default, this parameter is true.
 */
bool spinToPosition(float position, vex::rotationUnits units, bool waitForCompletion=true);
