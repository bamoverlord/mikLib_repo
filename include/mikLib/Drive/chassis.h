...
public:
    ...
    */
    ...
    void turn_encoder(float angle, float wheel_ratio, float wheel_diameter, float velocity_pct, const turn_to_angle_params& p);
    ...
    void drive_encoder(float distance, float wheel_ratio, float wheel_diameter, float velocity_pct, const drive_distance_params& p);
...
inline static float wheel_ratio{};
inline static float wheel_diameter{};
inline static float velocity_pct{};

inline void Chassis::drive_encoder(float distance, float _wheel_ratio, float _wheel_diameter, float _velocity_pct, const drive_distance_params& p = drive_distance_params()) {
    desired_distance = distance;
    desired_heading = p.heading;
    g_drive_distance_params_buffer = p;
    wheel_ratio = _wheel_ratio;
    wheel_diameter = _wheel_diameter;
    velocity_pct = _velocity_pct;
    ...
    // Calculate max voltage based on velocity percentage
    float max_voltage_from_velocity = 12.0 * (velocity_pct / 100.0);

    drive_task = vex::task([](){
        float max_voltage_from_velocity = 12.0 * (velocity_pct / 100.0);

        auto get_encoder_position = [](){
            const float drive_in_to_deg_ratio = wheel_ratio / 360.0 * M_PI * wheel_diameter;
            const float left = chassis.left_drive.position(vex::rotationUnits::deg) * drive_in_to_deg_ratio; 
            const float right = chassis.right_drive.position(vex::rotationUnits::deg) * drive_in_to_deg_ratio; 
            return (left + right) / 2;
        };
        ...
        float drive_start_position = get_encoder_position();
        float current_position = drive_start_position;
        ...
        while (chassis.pid.is_settled() == false) {
            ...
            // Use the velocity-based max voltage
            float actual_max_voltage = std::min(p.max_voltage, max_voltage_from_velocity);
            drive_output = clamp(drive_output, -actual_max_voltage, actual_max_voltage);
            heading_output = clamp(heading_output, -p.heading_max_voltage, p.heading_max_voltage);
...
inline void Chassis::turn_encoder(float angle, float _wheel_ratio, float _wheel_diameter, float _velocity_pct, const turn_to_angle_params& p = turn_to_angle_params{}) {
    desired_angle = angle;
    ...
    drive_task = vex::task([](){
        ...
        auto get_encoder_position = [](){
            const float turn_in_to_deg_ratio = wheel_ratio / 360.0 * M_PI * wheel_diameter;
            const float left = chassis.left_drive.position(vex::rotationUnits::deg) * turn_in_to_deg_ratio;
            const float right = chassis.right_drive.position(vex::rotationUnits::deg) * turn_in_to_deg_ratio;
            return (left - right) / 2;
        };
        ...
        float turn_start_position = get_encoder_position();
        float current_position = turn_start_position;

        float turn_error = angle - (current_position - turn_start_position);
        float prev_turn_error = turn_error;

        while (chassis.pid.is_settled() == false) {
            current_position = get_encoder_position();

            turn_error = angle + turn_start_position - current_position;
            chassis.distance_traveled += std::abs(turn_error - prev_turn_error);
            prev_turn_error = turn_error;

            float turn_output = chassis.pid.compute(turn_error);

            float actual_max_voltage = std::min(p.max_voltage, max_voltage_from_velocity);
            turn_output = clamp(turn_output, -actual_max_voltage, actual_max_voltage);

            turn_output = clamp_min_voltage(turn_output, p.min_voltage);

            chassis.drive_with_voltage(turn_output, -turn_output);
