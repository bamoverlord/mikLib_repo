...
bool mik::motor_group::spinToPosition(float position, vex::rotationUnits units, float voltage, vex::voltageUnits units_v, bool waitForCompletion) {
    float currentPos = this->position(units);
    float relativeMove = position - currentPos;
    return spinFor(relativeMove, units, voltage, units_v, waitForCompletion);
}

bool mik::motor_group::spinToPosition(float position, vex::rotationUnits units, bool waitForCompletion) {
    float currentPos = this->position(units);
    float relativeMove = position - currentPos;
    return spinFor(relativeMove, units, waitForCompletion);
}